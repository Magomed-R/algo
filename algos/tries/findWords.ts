export function findWords(board: string[][], words: string[]): string[] {
  const findedWords: string[] = []

  const check = (
    rest: string,
    row: number,
    col: number,
    used: boolean[][] = new Array(board.length)
      .fill(null)
      .map(() => new Array(board[0].length).fill(false))
  ): true | false => {
    if (rest[0] !== board[row][col]) return false
    ...
    for (let [dr, dc] of directions) {
      if (
        board[row + dr] &&
        board[row + dr][col + dc] === rest[1] &&
        !used[row + dr][col + dc] &&
        check(rest.slice(1), row + dr, col + dc, structuredClone(used))
      )
        return true
    }
  ...
  for (let word of words) {
    ...
    for (let row = 0; row < board.length; row++) {
      ...
      for (let col = 0; col < board[row].length; col++) {
        if (founded) break
        if (board[row][col] === word[0] && check(word, row, col)) {
